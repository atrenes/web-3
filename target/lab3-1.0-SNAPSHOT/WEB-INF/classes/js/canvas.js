...
drawAxis();
drawArea(document.getElementById("mainForm:select_r").value);
drawPointsFromTable(document.getElementById("mainForm:select_r").value);
function drawAxis() {
    let height = canvas.height;
    let width = canvas.width;

    ctx.fillStyle = "rgba(0, 0, 0, 0.05)";
    ctx.fillRect(width / 2 - 2 * i, height / 2 - 3 * i, 3.5 * i, i * 6);
...
function drawArea(r) {
    ...
    ctx.arc(width / 2, height / 2, r * i / 2,5 * Math.PI / 2, Math.PI, false);
...
function drawPointsFromTable(r) {
    ...
    let rData = window.document.getElementsByClassName("rdata");
    ...
    for (let i = 0 ; i < xData.length ; i++) {
        ...
        if (r === parseFloat(rData[i].innerHTML).toString()) {
